      ...
      >
        ...
        <div className='flex justify-between items-center pb-3 border-b border-neutral-100'>
          <div>
            ...
            <p className='text-[11px] text-neutral-400 mt-0.5'>Supervisión operativa de Fayver</p>
          </div>
          <button
            onClick={() => setIsHistoryOpen(false)}
        ...
        <div className='flex-1 overflow-y-auto py-3 space-y-2.5 custom-scrollbar pr-1'>
          {activities.length === 0 ? (
            <div className='text-center py-12 text-neutral-400 space-y-1.5'>
              <p className='text-xs font-semibold'>Tablón sin movimientos</p>
              <p className='text-[11px] text-neutral-300 px-4'>Las acciones que realices en esta sesión aparecerán aquí.</p>
            </div>
          ) : (
            activities.map(act => (
              <div
                key={act.id}
                className='text-[11px] bg-neutral-50/80 p-3 rounded-lg border border-neutral-100 shadow-2xs hover:bg-neutral-50 transition-colors animate-in fade-in slide-in-from-bottom-1 duration-200'
              >
                <p className='text-neutral-600 leading-relaxed font-medium break-words'>{act.text}</p>
                <span className='text-[9px] text-neutral-400 block mt-1.5 text-right font-mono'>{act.time}</span>
              </div>
            ))
          )}
        </div>
